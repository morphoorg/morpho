...
//real e() {return 2.718281828459045;}               // Euler's constant
...
//Vectors with values corresponding to (T2, HT, DT, T)
real[] atomic_masses() {      //Mass of non-tritium atom in each molecule (eV)
...
vector E_zp_values() {
vector[(size(atomic_masses())-1)] mu_values;
vector[(size(atomic_masses())-1)] hbar_omega_values;
vector[size(atomic_masses())] E_zp_values;
...
real find_delta_sigma_squared_epsilon(real temp, real p_squared, int num_J, real lambda, real kappa, real delta_epsilon){
...
delta_sigma_squared_epsilon <- (2.0*sigma_squared_i[1] - 2.0*kappa/(1.9+kappa)*sigma_squared_i[2] - 2.0/(1.0+kappa)*sigma_squared_i[3])*delta_epsilon;
...
real find_delta_sigma_squared_kappa(real temp, real p_squared, int num_J, real lambda, real epsilon, real kappa, real delta_kappa){
real sigma_squared_i[3];
...
delta_sigma_squared_kappa <- 2.0*(1.0-epsilon)/pow((1.0+kappa), 2)*(sigma_squared_i[2] - sigma_squared_i[3])*delta_kappa;
...
real find_delta_sigma(real temp, real p_squared, real delta_temp, int num_J, real delta_lambda, real[] composition, real lambda, real epsilon, real kappa, real delta_epsilon, real delta_kappa){
real delta_sigma_squared;
real delta_sigma;

delta_sigma_squared <- find_delta_sigma_squared_epsilon(temp, p_squared, num_J, lambda, kappa, delta_epsilon) + find_delta_sigma_squared_kappa(temp, p_squared, num_J, lambda, epsilon, kappa, delta_kappa);
