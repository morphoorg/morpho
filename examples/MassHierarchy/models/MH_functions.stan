...
vector matrix_elements(real sin_sq_2th12, real sin_sq_2th13) {
    ...
    cos_2th12 <- pow((1 - pow(sin_sq_2th12, 2)), 1/2);
    cos_2th13 <- pow((1 - pow(sin_sq_2th13, 2)), 1/2);

    sUe_fixed[1] <- 1/4*(1+cos_2th12)(1+cos_2th13);
    sUe_fixed[2] <- 1/4*(1-cos_2th12)(1+cos_2th13);
    sUe_fixed[3] <- 1/2*(1-cos_2th12);
...
real signal_to_noise_log(real KE, real Q, vector U, vector m_nu, real minKE, real maxKE, real signal_fraction) {
    ...
    for (i in 1:nFamily){
        if (KE < (Q-fabs(m_nu[i]))) {
                signal_log <- signal_log + log(signal_fraction) + log(U[i]) + get_beta_function_log(KE, Q, m_nu[i], minKE);
                    }
