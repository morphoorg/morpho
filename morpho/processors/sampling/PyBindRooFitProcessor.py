...
class PyFunctionObject(ROOT.TPyMultiGenFunction):
    ...
    def NDim( self ):
        logger.info('PYTHON NDim called')
...
class PyBindRooFitProcessor(RooFitInterfaceProcessor):
    ...
    def InternalConfigure(self, config_dict):
        ...
        self.module_name = reader.read_param(config_dict, "module_name", "required")
        self.function_name = reader.read_param(config_dict, "function_name", "required")
    ...
    def _defineDataset(self, wspace):
        ...
        for aVarName in self.ranges.keys():
            rooVarSet.add(ROOT.RooRealVar(str(aVarName), str(aVarName), min(self._data[aVarName]), max(self._data[aVarName])))
        print(rooVarSet)
    ...
    def definePdf(self, wspace):
        ...
        print(self.func)
        # self.pyFuncObj = PyFunctionObject(self.func)
        ...
        self.pdf = ROOT.RooFit.bindFunction("test",self.f, ROOT.RooArgList(*rooVarSet))
        print(self.pdf)
        # try:
